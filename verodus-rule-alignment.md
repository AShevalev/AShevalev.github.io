...
- Plan payout rules (same for every payout on that plan):
  - **Instant Qualified Performance:** **5 min trading days** for every payout (first and later). Say it simply. Do **not** mash Best Day into that line. Separate rule: **20% Best Day** — only days that close **more than 0.5%** net profit (vs that day’s start-of-day equity) count toward Positive Days’ Profit. Best Day must be ≤20% of that PDP. Exceeding 20% is not a breach — keep trading until Best Day is ≤20%. Instant has **no** 3-day QPP min.
...
- Cycle (same for the first payout and every payout after):
  ...
  - **Instant on-demand:** `$100` and **5 min trading days**.
  - **1-Step on-demand:** `$100` and **3 trading days** (50% Best Day still applies).
  - **2-Step Lite / Pro on-demand:** `$100` and **3 trading days**.
- Do not add a first-vs-later split (e.g. “first after 4 days, later after 3”). Instant 5 min trading days applies to the first reward and every reward after. The 3-day QPP min is 1-Step / 2-Step only.
...
<ul>
    ...
    <li>On-demand on Instant: $100 and 5 min trading days</li>
...
```text
Eligibility: You become eligible for a reward when net profit is at least $100, you have met 5 min trading days, and you have met the selected cycle. Weekly: 7 calendar days. Bi-weekly: 14 calendar days. On-demand: $100 and 5 min trading days. The first payout and every payout after use this same rule.
```
...
Instant `li10` / `span21` — **keep 5 min trading days.** Drop only “before your first reward.” Same 5 for every Instant payout. Do not put Best Day in this line.

```text
Minimum Trading Days: 5 min trading days.
```
...
On `trading-objectives.html`, Instant payout / on-demand lines use **5 min trading days** (every payout, not first-only). 1-Step / 2-Step lines use **3 trading days** (every payout, not first-only). Instant Best Day modal still gets the 0.5% day-count sentence.
...
## 5. Instant QPP — 5 min trading days (simple); Best Day stays separate
...
`instant.html` Eligibility is wrong today. It still uses the 1-Step / 2-Step QPP **3 trading days** sentence. Instant QPP is **5 min trading days**, first reward and every reward after. Say that simply. Best Day (20%, more than 0.5% counts) stays in the Best Day block / modal — not inside `li10`.

| Key | Live | Change to |
|---|---|---|
| `li23` / `span47` | At least **3 trading days** since QPP account or last payout | `$100` + **5 min trading days** + the selected cycle. Same first and later. |
| `li10` / `span21` | 5 valid days before your **first** reward | **5 min trading days.** Drop “first reward” only. |
...
|---|---|---|---|
| Instant | No eval. Funded from day one. | **5 min trading days** (same first and later). Separate: **20% Best Day**; only days **more than 0.5%** count toward PDP. **No** 3-day QPP min. | Weekly 7 calendar / bi-weekly 14 calendar / on-demand = `$100` + 5 min trading days |
...
A 1-Step / 2-Step “trading day” = open and close on the same calendar day. Instant QPP min days = **5 min trading days** (simple). Instant Best Day counted day = closes **more than 0.5%** net. Do not put Instant’s 0.5% floor on 1-Step. Do not put the 3-day QPP min on Instant. Do not put 2-Step’s **5 eval days** on 2-Step QPP payouts.
...
```html
<ul>
    <li>Instant: No evaluation phase. Qualified Performance: 5 min trading days (first payout and every payout after). Best Day ≤20%; only days that close more than 0.5% net profit count toward that rule.</li>
...
<ul>
    <li>Same rule for every payout (first and later): $100, that plan’s payout rule, and the cycle</li>
    <li>Instant: 5 min trading days, plus the cycle. No 3-day QPP min.</li>
...
<ul>
    ...
    <li>On Instant: 5 min trading days</li>
...
Cards already name news + no holding time. Instant QPP min days stay **5 min trading days** (simple). Best Day / 0.5% stays on the Instant rules page and modal, not in this min-days line. Optional Instant (`p7`):

```text
Every Instant payout (first and later) needs $100, 5 min trading days, and the selected cycle (weekly, bi-weekly, or on-demand).
```
...
- `content.li10` / `span21`: **5 min trading days.** Drop “before your first reward.” Same 5 for every payout. Do not add Best Day to this line.
- `content.p8` / `li12`–`li14`: add that only days **more than 0.5%** count toward Positive Days’ Profit.
- `content.li15`: 0.5% is the Best Day day-count, not a second min-days speech. Keep `li10` as the simple 5 min trading days line.
- Hero / `li4` / `li9`: Instant QPP min is **5 min trading days**. Best Day stays 20%.
- Eligibility `li23`: `$100` + **5 min trading days** + cycle. **No** 3-day QPP min.

### `trading-objectives.html` — other Instant / QPP lines (same pass)

| Live | Change to |
|---|---|
| Instant card “Minimum Trading Days” = **5 Days** (`instantCardHTML` / `pricing.fiveDays`) | **Keep.** Instant QPP is 5 min trading days. Drop “first only” if the card implies first payout only. |
| Instant data `minDays:'5'` | Keep. |
| `content.p8Instant`: First Payout after **5 trading days** | Every Instant payout: `$100` + **5 min trading days**. Drop “first only.” |
...
| Instant on-demand / “Anytime after min trading days” | Instant: `$100` + 5 min trading days. 1-Step / 2-Step: `$100` + 3 trading days. |
...
| Page | Apply |
|---|---|
| `instant.html` | QPP: **5 min trading days** (simple, first and later) in `li10` / Eligibility. 20% Best Day + more-than-0.5% count in the Best Day block **and** modal. Drop the 3-day QPP Eligibility sentence. `$100` + cycle. |
...
| `faq-plans.html` `p7` | Instant QPP: **5 min trading days** (simple). Best Day / 0.5% on the Instant rules page, not mashed into this line. |
| `faq-plans.html` 1-Step / 2-Step cards | Optional: QPP payouts need 3 trading days. Do not put Instant 0.5% on those cards. |
| `faq-qualified-trader.html` | Delete 4-then-3. Instant QPP = 5 min trading days. 1-Step / 2-Step QPP = 3 trading days. Same first and later. |
| `faq-evaluation.html` | Add Instant: no eval; QPP 5 min trading days. 1-Step: no min to pass; QPP 3 days. 2-Step: 5 per eval phase; QPP 3 days. |
| `performance-reward.html` | Instant = **5 min trading days**. “3 trading days” = **1-Step / 2-Step QPP only**. Drop first-only. |
| `terms.html` §8(b)/(c) | Keep “requirements vary by model.” Optional one line: Instant QPP needs 5 min trading days; Instant Best Day counted days need more than 0.5% net; 1-Step / 2-Step QPP payouts need 3 trading days; 2-Step eval needs 5. |
...
| Same rule first and later | n/a | n/a | Instant QPP: 5 min trading days (drop “first only”); 1-Step / 2-Step QPP: 3 trading days | n/a | n/a | Delete 4-then-3 list; rewrite `p9`/`p12` | n/a | Instant 5; 1-Step / 2-Step 3 |
...
- Instant QPP **5 min trading days** ≠ Instant **20% Best Day** (0.5% day-count) ≠ 1-Step / 2-Step QPP **3 trading days** ≠ 2-Step eval **5 trading days** ≠ 1-Step eval **no min days**. Do not mash Instant Best Day into the 5 min trading days line. First and later payouts on that plan use **that** rule **plus** the cycle.

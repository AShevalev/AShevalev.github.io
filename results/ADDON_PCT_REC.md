# Add-on % rec — profitable but competitive

PDF: `results/Verodus_Addon_Pct_BE_2026-08-17.pdf`. Leftover = `round(list × pct) × 0.52 − extra E[X]`. Prints if leftover ≥ −$1.

## Charge this

| Add-on | Instant | Evals | Instant $100k sticker | Why |
|---|---:|---:|---:|---|
| News | included | included | $0 | Not a SKU |
| Weekend Holding | **12%** | **12%** | $81 | Street Swing 10–15%. Live 18%. Instant BE 6%. |
| Weekly Rewards 70% | **8%** | **8%** | $54 | Live 6% is thin on Instant $100k. Under BG 7-day 15%. |
| On Demand Rewards 90% | **32%** | **15%** | $216 | Instant year-1 BE is 32%. Evals BE ~8%; 15% matches BG. |
| Bi-Weekly 80% | included | included | $0 | Default. Min $100. |

## Instant $100k math

List $675. Year-1 BE $272. Challenge leftover $26.

- Weekend extra $22 → BE 6% of list. Rec 12% leftover $20.
- Weekly speed extra $22 → BE 6%. Rec 8% leftover $6. Live 6% leftover $-0.
- On Demand 90% extra $112 → BE 32%. Rec 32% leftover $1. Checkout 20% leftover $-41 (combined $-15). BG 15% leftover $-59.

Pro $100k On Demand extra $20 → BE 7% of list. Rec 15% leftover $22.

Do not copy BG Instant 15% or FundedNext +5% for 90% + anytime. If Instant and evals must share one On Demand %, 25% is thin on Instant $100k combined.

(function () {
  ...
  var PRICING = {
    ...
    '2steplite': {
      label: '2-Step Lite',
      sizes: {
        '5000': { price: 60 }, '10000': { price: 85 }, '25000': { price: 177 },
        '50000': { price: 260 }, '100000': { price: 475 }, '200000': { price: 922 }
      }
    },
    '2steppro': {
      label: '2-Step Pro',
      sizes: {
        '5000': { price: 69 }, '10000': { price: 91 }, '25000': { price: 192 },
        '50000': { price: 306 }, '100000': { price: 537 }, '200000': { price: 1075 }
      }
    }
  };
  ...
  /* Default reward is Bi-Weekly 80% (not a toggle). News is included (no SKU).
     Weekly 70% is 6% of list. On Demand 90% is 32% Instant / 15% evals. */
  var PAYOUT_UPGRADES = [];
  var BUNDLES = [];
  ...
  var ADDONS = [
    ...
    },
    {
      ...
      tooltip: function () {
        return 'On Demand Rewards with 90% Split\n\nRequest your 90% reward share with no 7- or 14-day wait. Available when eligibility requirements are met.\n\n'
          + minDaysCopy();
      }
    }
  ];
  ...
  function minDaysCopy() {
    if (selectedPlan === 'instant') {
      return 'Eligibility already includes $100 and Best Day ≤20% of Positive Days’ Profit. Weekly / Bi-Weekly add the 7- or 14-day clock.';
    }
    if (selectedPlan === '1step') {
      return 'Eligibility already includes $100 and Best Day ≤50% of Positive Days’ Profit. Weekly / Bi-Weekly add the 7- or 14-day clock.';
    }
    return 'Eligibility already includes $100 and 3 trading days. Weekly / Bi-Weekly add the 7- or 14-day clock.';
  }
  ...
  function applyAddonToggle(id, checked) {
    if (checked) {
      if (PAYOUT_ADDON_EXCLUSIVE[id]) {
        Object.keys(PAYOUT_ADDON_EXCLUSIVE).forEach(function (other) {
          if (other !== id) delete activeAddons[other];
        });
      }
